...
#define MUDBUN_BRUSH_PINCHPULL 999 // 与 C# 中的 Type 值一致
...
)
{
  ...
  switch (brush.type)
  {
    ...
    case MUDBUN_BRUSH_PINCHPULL:
    {
      // Pinch/Pull distortion brush
      // Parameters packed in brush.data0 = (strength, radius, hardness, selfBlend)
      float  strength = brush.data0.x;
      float  R        = brush.data0.y;
      float  hardness = brush.data0.z;
      float  selfB    = brush.data0.w; // reserved if you need blending

      // Use brush-space coordinates for radial falloff
      float3 q = pRel;
      float  r = length(q.xy);
      float  d = saturate(1.0 - r / max(R, 1e-5));
      float  exponent = lerp(2.0, 8.0, hardness);
      float  falloff  = pow(d, exponent);

      // Displace along local +Z; since we don't have basis vectors here, we approximate by
      // modifying the world position along +Z. If your project exposes brush basis, convert it here.
      float3 disp = float3(0.0f, 0.0f, strength) * falloff;
      p += disp;
      break;
    }
  }
...
)
{
  ...
  switch (brush.type)
  {
    ...
    case MUDBUN_BRUSH_PINCHPULL:
    {
      // Bounds: box of half-size h, expanded by radius (data0.y)
      float R = brush.data0.y;
      res = sdf_box(pRel, h, R);
      break;
    }
  }
